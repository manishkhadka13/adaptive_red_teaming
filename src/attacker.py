...
class AdaptiveAttacker:
    ...
    def _generate(self, prompt: str, max_new_tokens: int = 1024, attempt_num: int = 0) -> str:
    ...
    def _cot_mutate(self, goal: str, prompt: str, refusal: str, previous: list, attempt_num: int = 0):
    ...
    def attack(self, goal: str, precision: str) -> AttackResult:
        ...
        for n in range(1, self.max_attempts + 1):
            ...
            # 2. Judge
            verdict = self.judge.evaluate(response=response, goal=goal)

            log.info(
                "  Judge: safety=%s refusal=%s",
                verdict.raw_label,
                verdict.refusal_label,
            )
